...
class OrderService {
  public getAll = async (query: OrderQueryType["query"]) => {
    try {
      ...
      // =========================
      const queryParams = {
        page: toInt((query as any).page, 1),
        limit: toInt((query as any).limit, 20),

        // opsional cursor (FE boleh abaikan)
        // format: "<createdAtISO>|<id>"
      ...
      // =========================
      const page =
        Number.isFinite(queryParams.page) && queryParams.page > 0
          ? queryParams.page
          : 1;

      const limitRaw =
        Number.isFinite(queryParams.limit) && queryParams.limit > 0
          ? queryParams.limit
          : 20;
      ...
      // page-based fallback (untuk FE lama)
      const skip = (page - 1) * limit;

      // cursor-based (untuk FE yang mau lebih efisien)
      const useCursorPagination = Boolean(queryParams.cursor);
      ...
      const cursorParsed = parseCursor(queryParams.cursor);

      // =========================
      // Build BASE filter (tanpa cursor)
      // (Ini juga dipakai buat totalItems agar selalu total keseluruhan)
      // =========================
      const baseFilter: OrderFilter = {};

      // Filter berdasarkan sales channel
      if (queryParams.salesChannelId) {
        baseFilter.salesChannelId = queryParams.salesChannelId;
      }
      ...
      if (queryParams.ordererCustomerId) {
        baseFilter.ordererCustomerId = queryParams.ordererCustomerId;
      }

      // Filter berdasarkan delivery target customer
      if (queryParams.deliveryTargetCustomerId) {
        baseFilter.deliveryTargetCustomerId =
          queryParams.deliveryTargetCustomerId;
      }

      // Filter berdasarkan delivery place
      if (queryParams.deliveryPlaceId) {
        baseFilter.deliveryPlaceId = queryParams.deliveryPlaceId;
      }

      // Filter berdasarkan ID order
      if (queryParams.orderId) {
        baseFilter.id = queryParams.orderId;
      }

      // =========================
      // OrderDetail-based filters (pindah ke DB supaya:
      // - tidak filter ulang di JS
      // - totalItems akurat
      // =========================
      // Build OrderDetail "is" filter (karena Order -> OrderDetail 1-1 opsional)
      // Jika ada filter terkait OrderDetail maka order harus punya OrderDetail (is: {...})
      const orderDetailIs: Prisma.OrderDetailWhereInput = {};

      // paymentStatus
      if (queryParams.paymentStatus) {
        orderDetailIs.paymentStatus = queryParams.paymentStatus;
      }

      // orderStatus mapping -> paymentStatus (jika dipakai)
      if (queryParams.orderStatus) {
        ...
        if (paymentStatus) {
          orderDetailIs.paymentStatus = paymentStatus;
        }
      }

      // unavailableReceipt => receiptNumber null
      if (queryParams.unavailableReceipt === "yes") {
        orderDetailIs.receiptNumber = null;
      }
      ...
      if (queryParams.receiptNumber) {
        orderDetailIs.receiptNumber = {
          contains: queryParams.receiptNumber,
          mode: "insensitive" as const,
        };
      }

      // paymentMethodId (sebelumnya difilter di JS via PaymentMethod !== null)
      if (queryParams.paymentMethodId) {
        orderDetailIs.paymentMethodId = queryParams.paymentMethodId;
      }

      // productId (sebelumnya difilter di JS via OrderProducts.length > 0)
      if (queryParams.productId) {
        orderDetailIs.OrderProducts = {
          some: {
      ...
      if (queryParams.productName) {
        // Jika sudah ada OrderProducts filter (productId), ini akan menimpa.
        // Gabungkan dengan AND supaya dua-duanya kepakai.
        const existingOP = orderDetailIs.OrderProducts;
        ...
        if (!existingOP) {
          orderDetailIs.OrderProducts = nameFilter as any;
        } else {
          orderDetailIs.AND = [
            ...(Array.isArray(orderDetailIs.AND)
              ? orderDetailIs.AND
              : orderDetailIs.AND
                ? [orderDetailIs.AND]
                : []),
            { OrderProducts: existingOP as any },
            { OrderProducts: nameFilter as any },
          ] as any;

          delete (orderDetailIs as any).OrderProducts;
        }
      }

      if (Object.keys(orderDetailIs).length > 0) {
        baseFilter.OrderDetail = { is: orderDetailIs };
      }

      // =========================
      // CustomerName / PhoneNumber filters (relasi)
      // =========================
      ...
      if (queryParams.phoneNumber) {
        ...
        if (!baseFilter.OR) {
          baseFilter.OR = [];
        } else if (!Array.isArray(baseFilter.OR)) {
      ...
      // =========================
      // Build DATA filter (base + cursor condition)
      // =========================
      const dataFilter: OrderFilter = { ...baseFilter };

      // cursor condition agar "data terbaru tampil teratas" dan bisa load next page dengan cursor:
      // orderBy: createdAt desc, id desc
      // untuk ambil halaman berikutnya (lebih lama):
      // - createdAt < cursorCreatedAt
      // - atau (createdAt == cursorCreatedAt && id < cursorId)
      if (useCursorPagination && cursorParsed) {
        const existingAnd = Array.isArray(dataFilter.AND)
          ? dataFilter.AND
          : dataFilter.AND
            ? [dataFilter.AND]
            : [];
        ...
        dataFilter.AND = [
          ...existingAnd,
          {
            OR: [
              ...
              {
                AND: [
                  { createdAt: { equals: cursorParsed.createdAt } },
      ...
      // =========================
      // Query: findMany (data) + count (total tanpa cursor)
      // =========================
      const [orders, totalItems] = await Promise.all([
        prismaClient().order.findMany({
          where: dataFilter,
          take: limit,
          // cursor mode tidak pakai skip; page mode tetap pakai skip
          skip: useCursorPagination ? 0 : skip,
          orderBy,
          include: {
            SalesChannel: true,
            DeliveryPlace: true,
            ...
            OrderDetail: {
              include: {
                // jika ada paymentMethodId, FE biasanya cuma butuh yang match itu
                // kalau tidak ada, ambil semua PaymentMethod detail
                PaymentMethod: queryParams.paymentMethodId
                  ? {
                      where: { id: queryParams.paymentMethodId },
                    }
                  : true,

                // jika productId ada, ambil orderProducts yg match itu saja
        ...
        prismaClient().order.count({ where: baseFilter }),
      ]);

      // =========================
      // Transform result (tetap seperti sebelumnya)
      // =========================
      ...
      // NOTE: Karena filter paymentMethodId/productId/paymentStatus sudah dipindah ke DB,
      // kita tidak perlu filter ulang di JS lagi.
      const filteredResult = result;

      // nextCursor (FE boleh abaikan)
      const last = filteredResult[filteredResult.length - 1];
      const nextCursor =
      ...
      const response = {
        ...
        meta: {
          ...
          // tambahan untuk cursor pagination (non-breaking)
          nextCursor,
          usedCursor: useCursorPagination,
        },
      };
  ...
  public create = async (data: CreateOrderType["body"]) => {
    ...
    try {
      ...
      if (data.order.ordererCustomerId) {
        ...
        if (!ordererCustomer) {
          return ServiceResponse.failure(
            "Data customer pemesan tidak ditemukan",
            null,
            StatusCodes.NOT_FOUND,
          );
        }
      ...
      if (data.order.deliveryPlaceId) {
        ...
        if (!deliveryPlace) {
          return ServiceResponse.failure(
            "Data tempat pengiriman tidak ditemukan",
            null,
            StatusCodes.NOT_FOUND,
          );
        }
      ...
      if (data.order.salesChannelId) {
        ...
        if (!salesChannel) {
          return ServiceResponse.failure(
            "Data saluran penjualan tidak ditemukan",
            null,
            StatusCodes.NOT_FOUND,
          );
        }
      ...
      if (data.orderDetail.detail) {
        ...
        if (data.orderDetail.detail.otherFees) {
          const { packaging, insurance, discount, ongkir1kg, installments } =
            data.orderDetail.detail.otherFees as {
              packaging?: number;
              insurance?: number;
              installments?: {
          ...
          if (packaging && typeof packaging !== "number") {
            return ServiceResponse.failure(
              "Biaya packaging harus berupa angka",
              null,
              StatusCodes.BAD_REQUEST,
            );
          }
          if (installments && typeof installments.amount !== "number") {
            return ServiceResponse.failure(
              "Biaya cicilan harus berupa angka",
              null,
              StatusCodes.BAD_REQUEST,
            );
          }
          if (insurance && typeof insurance !== "number") {
            return ServiceResponse.failure(
              "Biaya asuransi harus berupa angka",
              null,
              StatusCodes.BAD_REQUEST,
            );
          }
          ...
          if (data.orderDetail.detail.otherFees?.productDiscount) {
            ...
            if (!Array.isArray(productDiscount)) {
              return ServiceResponse.failure(
                "Diskon produk harus berupa array",
                null,
                StatusCodes.BAD_REQUEST,
              );
            }
            ...
            for (const discount of productDiscount) {
              if (
                !discount.produkVariantId ||
                typeof discount.produkVariantId !== "string"
              ) {
                return ServiceResponse.failure(
                  "ID varian produk harus valid",
                  null,
                  StatusCodes.BAD_REQUEST,
                );
              }
              ...
              if (typeof discount.discountAmount !== "number") {
                return ServiceResponse.failure(
                  "Jumlah diskon harus berupa angka",
                  null,
                  StatusCodes.BAD_REQUEST,
                );
              }
      ...
      if (productIds.length > 0) {
        const products = await prisma.product.findMany({
          where: {
            id: {
              in: productIds.filter((id): id is string => id !== undefined),
            },
          },
          select: { id: true },
        });
        ...
        const missingProductIds = productIds.filter(
          (id) => id !== undefined && !foundProductIds.includes(id),
        );
      ...
      if (
        data.orderDetail.shippingServices &&
        data.orderDetail.shippingServices.length > 0
      ) {
        for (const service of data.orderDetail.shippingServices) {
          if (!service.shippingName || !service.serviceName) {
            return ServiceResponse.failure(
              "Data shipping service tidak lengkap",
              null,
              StatusCodes.BAD_REQUEST,
            );
          }
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        for (const orderProduct of data.orderDetail.orderProducts) {
          ...
          if (orderProduct.productVariantId) {
            ...
            variant = product.productVariants.find(
              (v: any) => v.id === orderProduct.productVariantId,
            );
          } else {
          ...
          if (variant?.productPrices && variant.productPrices.length > 0) {
            ...
            switch (customerCategory) {
              ...
              case "MEMBER":
                productPrice =
                  Number(priceData.member) || Number(priceData.normal) || 0;
                break;
              case "RESELLER":
                productPrice =
                  Number(priceData.reseller) || Number(priceData.normal) || 0;
                break;
              case "AGENT":
                productPrice =
                  Number(priceData.agent) || Number(priceData.normal) || 0;
                break;
              default:
                productPrice = Number(priceData.normal) || 0;
            }
        ...
        if (data.orderDetail.detail.otherFees) {
          ...
          if (
            ...
            otherFees.installments.paymentMethodId &&
            otherFees.installments.amount
          ) {
              ...
              switch (customerCategory) {
                ...
                case "MEMBER":
                  discountProductPrice =
                    Number(productPriceDB.member) ||
                    Number(productPriceDB.normal) ||
                    0;
                  break;
                case "RESELLER":
                  discountProductPrice =
                    Number(productPriceDB.reseller) ||
                    Number(productPriceDB.normal) ||
                    0;
                  break;
                case "AGENT":
                  discountProductPrice =
                    Number(productPriceDB.agent) ||
                    Number(productPriceDB.normal) ||
                    0;
                  break;
                default:
          ...
          if (otherFees.discount?.value) {
            if (otherFees.discount.type === "percent") {
              totalPrice -=
                (totalPrice * Number(otherFees.discount.value)) / 100;
            } else {
        ...
        for (const orderProduct of orderProductDataList) {
          ...
          if (
            ...
            productVariant.stock < orderProduct.productQty
          ) {
            throw new Error(
              `Stok produk dengan ID ${orderProduct.productVariantId} tidak mencukupi`,
            );
          }
        ...
        if (
          ...
          const shippingServicesPromises =
  ...
  public update = async (
    id: string,
    data: Partial<UpdateOrderType>["body"],
  ) => {
    try {
      let totalPrice = 0;
      ...
      if (
        ...
        data.orderDetail.orderProducts.length > 0
      ) {
        ...
        for (const orderProduct of data.orderDetail.orderProducts) {
          ...
          if (orderProduct.productVariantId) {
            const matchingVariant = product.productVariants.find(
              (v) => v.id === orderProduct.productVariantId,
            );
            if (matchingVariant) {
          ...
          if (variant?.productPrices && variant.productPrices.length > 0) {
            ...
            if (existingOrder?.OrderDetail?.OrderProducts) {
              const existingProduct =
                existingOrder.OrderDetail.OrderProducts.find(
                  (p) =>
            ...
            const finalQty =
              Number(productQty) || Number(existingProductQty) || 1;
        ...
        if (data.orderDetail.detail?.otherFees) {
          ...
          if (otherFees.installments) {
            if (
              ...
              const existingInstallment =
                await prismaClient().installment.findFirst({
                  where: {
                    orderId: id,
                    paymentMethodId: otherFees.installments.paymentMethodId,
                  },
          ...
          if (otherFees.discount) {
            if (
              otherFees.discount.type === "percent" &&
              otherFees.discount.value
            ) {
              const discountAmount =
                (totalPrice * Number(otherFees.discount.value)) / 100;
              totalPrice -= discountAmount;
            } else if (
              otherFees.discount.type === "nominal" &&
              otherFees.discount.value
            ) {
              totalPrice -= Number(otherFees.discount.value);
            }
          ...
          if (
            ...
            for (const discount of otherFees.productDiscount) {
              ...
              if (product) {
                const productPriceDB =
                  await prismaClient().productPrice.findFirst({
                    where: { productVariantId: product.productVariantId },
                  });
      ...
      if (data?.orderDetail) {
        const orderDetailData = {
          ...
          otherFees: data.orderDetail.detail?.otherFees
            ? data.orderDetail.detail.otherFees
            : undefined,
          originalFinalPrice: Number(totalPrice),
          finalPrice: data.orderDetail.detail?.originalFinalPrice
            ? Number(data.orderDetail.detail.originalFinalPrice)
      ...
      if (
        ...
        const orderProductPromises = data.orderDetail.orderProducts.map(
          async (orderProduct) => {
            ...
            if (orderProduct.productVariantId) {
              ...
              if (!productVariant) {
                throw new Error(
                  `Varian produk dengan ID ${orderProduct.productVariantId} tidak ditemukan`,
                );
              }
              ...
              const previousQty = existingProduct
                ? existingProduct.productQty
                : 0;
              ...
              if (
                productVariant.stock !== null &&
                qtyDifference > 0 &&
                productVariant.stock < qtyDifference
              ) {
              ...
              if (qtyDifference !== 0) {
                ...
                if (qtyDifference > 0) {
                  console.log(
                    `Mengurangi stok untuk variant ${orderProduct.productVariantId} sebanyak ${qtyDifference}`,
                  );
                  ...
                  const newStock =
                    (currentVariant?.stock || 0) + Math.abs(qtyDifference);
                  console.log({ newStock });
                  // Update stok dengan nilai absolut
                  await prismaClient().productVariant.update({
                    where: { id: orderProduct.productVariantId },
      ...
      return ServiceResponse.success(
        "Berhasil mengupdate data order",
        result,
        StatusCodes.OK,
      );
    } catch (error) {
  ...
  public delete = async (id: string) => {
    try {
      ...
      const result = await prismaClient().$transaction(async (prisma) => {
        ...
        if (
          existingOrder.OrderDetail?.OrderProducts &&
          existingOrder.OrderDetail.OrderProducts.length > 0
        ) {
    ...
    try {
      ...
      return exportData<OrderWithRelations>(
                      ...
                      ? {
                          where: {
                            ...
                            ...(queryParams.paymentStatus && {
                              status: queryParams.paymentStatus,
                            }),
                          },
                        }
                      : true,
        ...
        (order: OrderWithRelations, index: number) => {
          ...
          const shippingCost =
            order.OrderDetail?.otherFees?.shippingCost?.cost ?? 0;
          const otherFees =
          ...
          if (order.OrderDetail) {
            ...
            if (order.OrderDetail.otherFees?.discount) {
              if (order.OrderDetail.otherFees.discount.type === "percent") {
                diskonValue =
                  (totalHarga * order.OrderDetail.otherFees.discount.value) /
                  100;
              } else {
          ...
          return {
            ...
            "Tanggal Order": order.orderDate
              ? formatter.format(new Date(order.orderDate))
              : "",
            ...
            "Layanan Pengiriman":
              order.ShippingServices?.[0]?.serviceName ?? "",
            "Tipe Pengiriman":
              order.OrderDetail?.otherFees?.shippingCost?.type ?? "",
            Catatan: order.note ?? "",
    ...
    try {
      const importResult = await importData<Prisma.OrderWhereInput>(
        ...
        <T>(row: Record<string, unknown>, index: number): T => {
          ...
          const discountType = row.Discount?.toString().includes("%")
            ? "percent"
            : "fixed";
          const discountValue = Number(
            row.Discount?.toString().replace("%", ""),
          );
        ...
        async (data) => {
          for (const item of data) {
            ...
            if (existingOrder) {
              console.log(
                `Order dengan kode ${item.OrderDetail?.code} sudah ada, dilewati.`,
              );
              continue;
            }
            ...
            if (!deliveryTargetCustomer) {
              const newDeliveryTargetCustomer =
            ...
            if (!deliveryPlace) {
              const newDeliveryPlace =
                await prismaClient().deliveryPlace.create({
                  data: {
                    id: uuidv4(),
                    name: item.OrdererCustomer as string,
                    address: "",
            ...
            const products = await Promise.all(
              (
                item.OrderDetail?.OrderProducts as {
                  productName: string;
                  skus: string[];
                  quantity: number;
                }[]
              ...
              data: {
                ...
                OrderDetail: {
                  create: {
                    ...
                    paymentStatus:
                      (item.OrderDetail?.paymentStatus as PaymentStatus) ||
                      "PENDING",
                    paymentDate:
                    ...
                    receiptNumber:
                      (item.OrderDetail?.receiptNumber as string) || null,
                    otherFees: item.OrderDetail?.otherFees as
